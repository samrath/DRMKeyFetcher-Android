 ...
 */
@TargetApi(18)
public class DRMDefaultDrmSessionManager<T extends ExoMediaCrypto> implements DrmSessionManager<T>,
        DRMDefaultDrmSession.ProvisioningManager<T> {
    ...
    private final List<DRMDefaultDrmSession<T>> sessions;
    private final List<DRMDefaultDrmSession<T>> provisioningSessions;

    private Looper playbackLooper;
    private int mode;
    private byte[] offlineLicenseKeySetId;

    public DRMDefaultDrmSessionEventListener getDRMDefaultDrmSessionEventListener() {
        return DRMDefaultDrmSessionEventListener;
    }

    public void setDRMDefaultDrmSessionEventListener(DRMDefaultDrmSessionEventListener DRMDefaultDrmSessionEventListener) {
        this.DRMDefaultDrmSessionEventListener = DRMDefaultDrmSessionEventListener;
    }

    DRMDefaultDrmSessionEventListener DRMDefaultDrmSessionEventListener;

    /* package */ volatile DRMDefaultDrmSessionManager.MediaDrmHandler mediaDrmHandler;
     ...
     */
    @Deprecated
    public static DRMDefaultDrmSessionManager<FrameworkMediaCrypto> newWidevineInstance(
            MediaDrmCallback callback,
            HashMap<String, String> optionalKeyRequestParameters,
            Handler eventHandler,
            DefaultDrmSessionEventListener eventListener)
            throws UnsupportedDrmException {
        DRMDefaultDrmSessionManager<FrameworkMediaCrypto> drmSessionManager =
    ...
    /**
     * @deprecated Use { #newPlayReadyInstance(MediaDrmCallback, String)} and {@link
     *     #addListener(Handler, DefaultDrmSessionEventListener)}.
     */
    @Deprecated
    public static DRMDefaultDrmSessionManager<FrameworkMediaCrypto> newPlayReadyInstance(
            MediaDrmCallback callback,
            String customData,
            Handler eventHandler,
            DefaultDrmSessionEventListener eventListener)
     ...
     */
    @Deprecated
    public static DRMDefaultDrmSessionManager<FrameworkMediaCrypto> newFrameworkInstance(
            UUID uuid,
            MediaDrmCallback callback,
            HashMap<String, String> optionalKeyRequestParameters,
            Handler eventHandler,
    ...
    public static DRMDefaultDrmSessionManager<FrameworkMediaCrypto> newFrameworkInstance(
            ...
            throws UnsupportedDrmException {
        return new DRMDefaultDrmSessionManager<>(
                uuid,
    ...
    /**
     * @deprecated Use {@link #DRMDefaultDrmSessionManager(UUID, ExoMediaDrm, MediaDrmCallback, HashMap)}
     *     and {@link #addListener(Handler, DefaultDrmSessionEventListener)}.
     */
    @Deprecated
    public DRMDefaultDrmSessionManager(
            UUID uuid,
    ...
    public DRMDefaultDrmSessionManager(
        ...
        mediaDrm.setOnEventListener(new DRMDefaultDrmSessionManager.MediaDrmEventListener());
    }
    ...
    /**
     * Provides access to { ExoMediaDrm#getPropertyString(String)}.
     * <p>
    ...
    /**
     * Provides access to { ExoMediaDrm#setPropertyString(String, String)}.
     * <p>
     * This method may be called when the manager is in any state.
     *
    ...
    /**
     * Provides access to { ExoMediaDrm#getPropertyByteArray(String)}.
     * <p>
     * This method may be called when the manager is in any state.
     *
     * @param key The key to request.
    ...
    /**
     * Provides access to {ExoMediaDrm#setPropertyByteArray(String, byte[])}.
     * <p>
     * This method may be called when the manager is in any state.
     *
     * @param key The property to write.
     ...
     */
    public void setMode(@DRMDefaultDrmSessionManager.Mode int mode, byte[] offlineLicenseKeySetId) {
    ...
    public DrmSession<T> acquireSession(Looper playbackLooper, DrmInitData drmInitData) {
        ...
        if (sessions.isEmpty()) {
            ...
            if (mediaDrmHandler == null) {
                mediaDrmHandler = new DRMDefaultDrmSessionManager.MediaDrmHandler(playbackLooper);
            }
        }
        ...
        if (offlineLicenseKeySetId == null) {
            ...
            if (schemeDatas.isEmpty()) {
                final DRMDefaultDrmSessionManager.MissingSchemeDataException error = new DRMDefaultDrmSessionManager.MissingSchemeDataException(uuid);
                if (DRMDefaultDrmSessionEventListener !=null){
                    DRMDefaultDrmSessionEventListener.onDrmSessionManagerError(error);
                }
                return new ErrorStateDrmSession<>(new DrmSession.DrmSessionException(error));
            }
        }

        DRMDefaultDrmSession<T> session;
        if (!multiSession) {
            session = sessions.isEmpty() ? null : sessions.get(0);
        } else {
            // Only use an existing session if it has matching init data.
            session = null;
            for (DRMDefaultDrmSession<T> existingSession : sessions) {
        ...
        if (session == null) {
            ...
            session =
                    new DRMDefaultDrmSession<>(
                            ...
                            eventDispatcher,
                            initialDrmRequestRetryCount);
            session.setDRMDefaultDrmSessionEventListener(DRMDefaultDrmSessionEventListener);
    ...
    public void releaseSession(DrmSession<T> session) {
        ...
        DRMDefaultDrmSession<T> drmSession = (DRMDefaultDrmSession<T>) session;
    ...
    @Override
    public void provisionRequired(DRMDefaultDrmSession<T> session) {
    ...
    public void onProvisionCompleted() {
        for (DRMDefaultDrmSession<T> session : provisioningSessions) {
    ...
    private class MediaDrmHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            for (DRMDefaultDrmSession<T> session : sessions) {
    ...
    private class MediaDrmEventListener implements ExoMediaDrm.OnEventListener<T> {
        ...
        public void onEvent(ExoMediaDrm<? extends T> md, byte[] sessionId, int event, int extra,
                            byte[] data) {
            if (mode == DRMDefaultDrmSessionManager.MODE_PLAYBACK) {
